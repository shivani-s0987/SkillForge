...
const StudentFilters = ({ query, onQueryChange, onApply }) => {
  ...
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} className="mt-4 bg-white p-4 rounded-lg shadow flex flex-col lg:flex-row lg:items-center lg:justify-between">
      <div className="flex items-center space-x-3">
        <input value={query} onChange={e=>onQueryChange && onQueryChange(e.target.value)} onKeyDown={e=>{ if(e.key==='Enter'){ onApply && onApply() } }} placeholder="Search students, emails..." className="px-3 py-2 border rounded w-64" />
        <select value={course} onChange={e=>setCourse(e.target.value)} className="px-3 py-2 border rounded">
          <option value="all">All Courses</option>
          <option value="course1">Course 1</option>
        </select>
        <button onClick={()=> onApply && onApply()} className="bg-indigo-600 text-white px-4 py-2 rounded">Apply</button>
      </div>
